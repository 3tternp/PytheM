    ...
    def start(self):
        try:
            ...
            for i in range(15):
                time.sleep(0.1)
                sys.stdout.write(
                    "\r" + "[" + animation[i % len(animation)] + "]" + "PytheM is loading ...")
                sys.stdout.flush()
            print() #Untill break or CTRL+C
            while 1:
                ...
                try:
                    ...
                    if self.command == "help":
                        ...
                        if self.arpspoof_status:
                            try:
                                ...
                                with open("{}/log/bdfproxy.log".format(self.path), "a+") as stdout:
                                    self.p1 = subprocess.Popen([sys.executable, "{}/third-party/BDFProxy/bdf_proxy.py".format(self.path), "bdfproxy"], stdout=stdout, stderr=stdout)
                        ...
                        if self.arpspoof_status:
                            try:
                                ...
                                with open("{}/log/sslstrip.log".format(self.path), "a+") as stdout:
                                    self.p1 = subprocess.Popen([sys.executable, "{}/third-party/sslstrip2/sslstrip.py".format(self.path), "-l", "8000"], stdout=stdout, stderr=stdout)
                                self.sslstrip_status = True
                                with open("{}/log/dns2proxy.log".format(self.path),"a+") as stdout:
                                    self.p2 = subprocess.Popen([sys.executable, "{}/third-party/dns2proxy/dns2proxy.py".format(self.path)], stdout=stdout, stderr=stdout)
                        ...
                        try:
                            if self.input_list[1] == "interface":
                                ...
                                try:
                                    ...
                                    try:
                                        self.domain = input("[+] Domain to be spoofed: ")
                                    except KeyboardInterrupt:
                                        pass
                                ...
                                try:
                                    ...
                                    try:
                                        self.redirect = input("[+] IP address to redirect DNS queries: ")
                                    except KeyboardInterrupt:
                                        pass
                                ...
                                try:
                                    ...
                                    try:
                                        self.script = input("[+]Script url/path: ")
                                    except KeyboardInterrupt:
                                        pass
                                ...
                                try:
                                    ...
                                    try:
                                        self.gateway = input("[+] Enter the gateway: ")
                                    except KeyboardInterrupt:
                                        pass
                                ...
                                try:
                                    ...
                                    try:
                                        self.arpmode = input("[+] Enter the arpmode: ")
                                    except KeyboardInterrupt:
                                        pass
                                ...
                                try:
                                    ...
                                    try:
                                        self.filter = input("[+] Enter the sniffer filter: ")
                                    except KeyboardInterrupt:
                                        pass
                        ...
                        try:
                            ...
                            if self.input_list[1] == "start":
                                ...
                                if self.domain != None :
                                    ...
                                    try:
                                        domain = input("[!] Type all to spoof all domains\n[+] Domain to be spoofed: ")
                                        self.domain = domain
                                ...
                                if self.redirect != None:
                                    ...
                                    myip = get_myip(self.interface)
                                    opt = input("[+] Default address to redirect is:{} do you want to change?[y/n]".format(myip))
                                    if opt == "y" or opt == "Y" or opt == "yes" or opt == "YES": 
                                        try:redirect = input("[+] IP address to be redirected: ")
                                        except KeyboardInterrupt: pass
                                    else:
                                        redirect = myip
                        ...
                        try:
                            ...
                            try:
                                self.filter = input("[+] Enter the filter(empty for core sniffer): ")
                                if self.filter == "dns":
                                    self.filter = "port 53"
                                if not self.filter:
                                    self.filter = None
