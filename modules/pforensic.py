...
class PcapReader(object):
    ...
    color(
        ...
        print(
    color(
        "[*] count :        Display how much packets the .pcap file have.",
         "blue"))
    ...
    def custom_filter(self, packets, filter):
        ...
        if filter == "string":
            ...
            try:
                find = input(
                    "[+] String to search on packets (case sensitive): ")
                for p in packets:
                    capture = StringIO()
            ...
            try:
                find = input("[+] Layer to search on packets (uppercase): ")
                for p in packets:
                    pkt = "\r\n\n\n------------------------[Packet n:{}]------------------------\r\n".format(
                        x)
                    end = "\r\n------------------------------------------------------------\r\n"
        ...
        while True:
            try:
                ...
                try:
                    ...
                    elif self.input_list[0] == 'exit' or self.input_list[0] == 'quit':
                        break
