...
class Scanner(object):
    ...
    def __init__(self, target, interface, mode, ports=None):
     ...
     65000]
        ...
        if ports is not None:
            if isinstance(ports, (list, tuple)):
                ...
                self.portManual = ",".join(str(p) for p in self.ports)
            else:
                self.portManual = str(ports)
                self.ports = [int(p.strip()) for p in self.portManual.split(",") if p.strip()]
                    ...
                    if(resp.getlayer(TCP).flags == 0x12):
                        ...
                        try:
                            if dstPort == 80 or dstPort == 8080:
                                ...
                                resp = s.recv(1024)
                                if resp:
                                    resp,garbage = resp.decode(errors="replace").split("\r\n\r\n", 1)
                                    for l in resp.split("\n"):
                                        print("       |{}".format(l))
                                s.close()
                                ...
                                s.connect((self.targetip,dstPort))
                                resp = s.recv(1024)
                                for l in resp.decode(errors="replace").split("\n"):
                                    print("       |{}".format(l))
                                s.close()
    ...
    def MANUALscanner(self):
        liveCounter = 0
        if not self.portManual:
            self.portManual = input("[+] Enter the port, ports (separated by commas): ")
        if not self.ports:
            self.ports = []
