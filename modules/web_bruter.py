...
class WEBbrutus(object):
    ...
    def build_wordlist(self, wordlist):
        ...
        fd = open(self.wordlist, "r", encoding="utf-8", errors="ignore")
    ...
    def form_bruter(self):
        try:
            self.login = input("[+] Enter the input name of the username box: ")
            self.psswd = input("[+] Enter the input name of the password box: ")
            self.user = input("[+] Enter the username to brute-force the formulary: ")
        ...
        while not self.word_queue.empty():
            ...
            try:
                for brute in attempt_list:
                    url = "%s%s" % (self.target_url,urllib.parse.quote(brute))
                    try:
                        headers = {}
